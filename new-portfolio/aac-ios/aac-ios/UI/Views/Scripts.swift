...
struct ScriptTextScreen: View {
    ...
    @StateObject private var scriptGenerationViewModel = ScriptGenerationViewModel()
    ...
    var body: some View {
        VStack(spacing:30) {
            ...
            // Generate/Refresh Scripts Button
            Button(action: {
                Task {
                    await scriptGenerationViewModel.refreshScripts()
                }
            }) {
                if scriptGenerationViewModel.isGenerating {
                    HStack {
                        ProgressView()
            ...
            .disabled(scriptGenerationViewModel.isGenerating)
            
            // Error message display
            if let errorMessage = scriptGenerationViewModel.errorMessage {
                Text(errorMessage)
                    .foregroundColor(.red)
                    .font(.system(size: 18))
                    .padding()
            ...
            // Script text fields
            ForEach(0..<scriptGenerationViewModel.scripts.count, id: \.self) { index in
                HStack {
                    HStack {
                        TextField("Enter script here", text: Binding(
                            get: { 
                                index < scriptGenerationViewModel.scripts.count ? scriptGenerationViewModel.scripts[index] : ""
                            },
                            set: { newValue in
                                scriptGenerationViewModel.updateScript(newValue, at: index)
                            }
                        ), axis: .vertical)
                        .font(.system(size: 24))
                        .padding(10)
                        ...
                        ZStack{
                            ...
                            Image(systemName: "speaker.wave.2.circle.fill")
                                ...
                                .frame(width: 30, height: 30)
                                .onTapGesture {
                                    let scriptText = index < scriptGenerationViewModel.scripts.count ? scriptGenerationViewModel.scripts[index] : ""
                                    speakText(text: scriptText)
                                }
                        }.padding()
                    }
                    .background(RoundedRectangle(cornerRadius: 10).fill(scriptGenerationViewModel.scripts[index].isEmpty ? .white : Color("AACBlue")))
                    .overlay(RoundedRectangle(cornerRadius: 10).stroke(Color(.black), lineWidth: 1))
                }
            }
            
            Button(action: {
                // Save scripts using the view model
                scriptGenerationViewModel.saveScripts(for: currScriptLabel)
                
                // Update global categoryTexts for compatibility
                categoryTexts[currScriptLabel] = scriptGenerationViewModel.scripts
                scriptsViewModel.saveScripts(categoryTexts)
        ...
        .padding(30)
        .task {
            // Fetch scripts when the view appears
            await scriptGenerationViewModel.fetchScripts(for: currScriptLabel)
        }
        .onDisappear {
            // Clear the view model when the view disappears
            scriptGenerationViewModel.clear()
        }
    }
